...
class SaleOrderInherit(models.Model):
    ...
    def _compute_page_sale_product_hide(self):
        ...
        if not res_user.has_group('nck_product_documents.group_users_access') and not res_user.has_group(
                'nck_product_documents.group_admin_access'):
    ...
    def _compute_get_user_product(self):
        if not self.page_sale_product_hide:
            res_user = self.env['res.users'].search([('id', '=', self.env.user.id)])
            if res_user.has_group('nck_product_documents.group_users_access') and not res_user.has_group(
                    'nck_product_documents.group_admin_access'):
