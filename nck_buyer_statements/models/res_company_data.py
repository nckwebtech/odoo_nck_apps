...
class ResCompanyData(models.Model):
    ...
    def _onchange_res_company_settings(self):
        if self.send_overdue_statement:
            scheduled_data_reference = self.env.ref('nck_buyer_statements.cron_send_buyer_overdue_statements')
        ...
        if self.customer_statement_bool and self.auto_statement_monthly:
            scheduled_data_reference = self.env.ref('nck_buyer_statements.cron_send_buyer_monthly_statement')
