...
class ResBuyerSellerData(models.Model):
    ...
    def action_get_buyer_filtered_statements(self):
        ...
        view = self.env.ref('nck_buyer_statements.view_filter_buyer_statements')
        return {
    ...
    def action_get_supplier_filtered_statements(self):
        ...
        view = self.env.ref('nck_buyer_statements.view_filter_supplier_statements')
        return {
    ...
    def action_print_buyer_statement(self):
        self.ensure_one()
        report = self.env.ref('nck_buyer_statements.action_report_buyer_statements').report_action(self, data={
            'partner_id': self})
        print("report------------------",report)
    ...
    def action_show_monthly_statements(self):
        self.ensure_one()
        report = self.env.ref('nck_buyer_statements.action_report_buyer_monthly_statements').report_action(self, data={
            'partner_id': self})
        if report.get('report_name'):
            base_url = self.get_base_url()
            self.update(
    ...
    def action_show_weekly_statements(self):
        self.ensure_one()
        report = self.env.ref('nck_buyer_statements.action_report_buyer_weekly_statements').report_action(self, data={
            'partner_id': self})
        if report.get('report_name'):
            base_url = self.get_base_url()
            self.update(
    ...
    def action_print_filtered_buyer_statement(self):
        self.ensure_one()
        report = self.env.ref('nck_buyer_statements.action_report_buyer_filtered_statements').report_action(self, data={
            'partner_id': self})
        if report.get('report_name'):
            base_url = self.get_base_url()
            self.update(
    ...
    def action_print_buyer_overdue_statement(self):
        self.ensure_one()
        report = self.env.ref('nck_buyer_statements.action_report_buyer_overdue_statements').report_action(self, data={
            'partner_id': self})
        if report.get('report_name'):
            base_url = self.get_base_url()
            self.update(
    ...
    def action_print_supplier_statement(self):
        ...
        return self.env.ref('nck_buyer_statements.action_report_supplier_filtered_statements').report_action(self,
                                                                                                             data=data)
    ...
    def action_sent_buyer_mail(self):
        ...
        return {'type': 'ir.actions.act_window',
                ...
                'target': 'new',
                'view_id': self.env.ref('nck_buyer_statements.partner_email_wizard_wizard_form').id,
                'view_mode': 'form',
    ...
    def action_show_custom_wizard(self):
        return {'type': 'ir.actions.act_window',
                ...
                'target': 'new',
                'view_id': self.env.ref('nck_buyer_statements.wizard_form_custom_statement_conf').id,
                'view_mode': 'form',
    ...
    def _sent_custom_statement_email(self):
        ...
        for partner_record in self:
            ...
            if partner_record.email:
                template = self.env.ref('nck_buyer_statements.custom_buyer_statement_email_template')
                data = self.env.ref('nck_buyer_statements.action_report_buyer_custom_statements').report_action(self,
                                                                                                                data={
    ...
    def sent_overdue_cron_buyer_statements(self):
        ...
        if self.buyer_amount_overdue_payment != 0.00 and self.email:
            ...
            template = overdue_email_template_id if overdue_email_template_id else self.env.ref(
                'nck_buyer_statements.buyer_overdue_statement_email_template')
        ...
        for buyer in self:
            if not buyer.exclude_statements:
                if buyer.email and buyer.buyer_amount_due_payment_monthly != 0.00:
                    ...
                    if not template:
                        template = self.env.ref('nck_buyer_statements.monthly_buyer_statement_email_template')
                    data = self.env.ref('nck_buyer_statements.action_report_buyer_monthly_statements')
                    if data['report_name']:
        ...
        for buyer in self:
            if not buyer.exclude_statements and not buyer.exclude_weekly_statements:
                if buyer.email and buyer.buyer_amount_due_payment_weekly != 0.00:
                    ...
                    data = self.env.ref('nck_buyer_statements.action_report_buyer_weekly_statements')
                    if data['report_name']:
                        base_url = buyer.get_base_url()
                        buyer.update(
                            {'qr_code_url': base_url + '/report/statements/page/%i' % buyer.id + '/' + '%s' % data[
