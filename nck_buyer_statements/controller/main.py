...
class CustomerPortalStatements(PortalAccount):
    ...
    def portal_statements_datas(self, partner=None, filename=None, **kw):
        ...
        if filename == 'nck_buyer_statements.report_buyer_statements':
            docids = [partner_id.id]
            report_datas = request.env['report.nck_buyer_statements.report_buyer_statements'].sudo()._get_report_values(
            ...
            return request.render('nck_buyer_statements.portal_my_statement_invoices', report_datas)

        if filename == 'nck_buyer_statements.report_buyer_filtered_statements':
            docids = [partner_id.id]
            report_datas = request.env[
                'report.nck_buyer_statements.report_buyer_filtered_statements'].sudo()._get_report_values(
                docids, data={'partner_id': partner_id})
        ...
        if filename == 'nck_buyer_statements.report_buyer_weekly_statements':
            docids = [partner_id.id]
            report_datas = request.env[
                'report.nck_buyer_statements.report_buyer_weekly_statements'].sudo()._get_report_values(
                docids, data={'partner_id': partner_id})
        ...
        if filename == 'nck_buyer_statements.report_buyer_monthly_statements':
            docids = [partner_id.id]
            report_datas = request.env[
                'report.nck_buyer_statements.report_buyer_monthly_statements'].sudo()._get_report_values(
                docids, data={'partner_id': partner_id})
        ...
        if filename == 'nck_buyer_statements.report_buyer_custom_statements':
            docids = [partner_id.id]
            report_datas = request.env[
                'report.nck_buyer_statements.report_buyer_custom_statements'].sudo()._get_report_values(
                docids, data={'partner_id': partner_id})
        ...
        if filename == 'nck_buyer_statements.report_buyer_overdue_statements':
            docids = [partner_id.id]
            report_datas = request.env[
                'report.nck_buyer_statements.report_buyer_overdue_statements'].sudo()._get_report_values(
                docids, data={'partner_id': partner_id})
    ...
    def portal_pay_multi_invoices(self, partner=None, **kw):
        ...
        if not selected_list:
            ...
            values = self._multiple_invoice_get_page_view_values(invoices_sudo, access_token, amount, currency[0],
                                                                 partner_data, **kw)
            return request.render("nck_buyer_statements.portal_invoice_payment_option", values)
    ...
    def portal_multi_payment_success(self, **kw):
        ...
        return request.render("nck_buyer_statements.portal_invoice_payment_success", {})
